...
RUN cd /usr/local/src/ruby-2.3.1 \
...
RUN cd /usr/local/src/yaml-0.1.7/ && ./configure --prefix=/usr/local \
    && make -j8 && make install

# APR
ADD apr-1.5.2.tar.gz /usr/local/src
RUN cd /usr/local/src/apr-1.5.2/ && ./configure --prefix=/usr/local/apr-1.5.2 \
    && make -j8 && make install

# APR-util
ADD apr-util-1.5.4.tar.gz /usr/local/src
RUN cd /usr/local/src/apr-util-1.5.4 && ./configure --prefix=/usr/local/apr-util --with-apr=/usr/local/apr-1.5.2 \
    && make -j8 && make install
...
RUN cd /usr/local/src/httpd-2.4.23 && ./configure --enable-shared=max --enable-module=all \
    --enable-systemd --prefix=/usr/local/apache --with-apr=/usr/local/apr-1.5.2 --with-apr-util=/usr/local/apr-util \
    --with-pcre=/usr/local/pcre && make -j8 && make install

RUN echo 'LD_LIBRARY_PATH=/usr/local/pgsql/lib:/usr/local/apr-1.5.2/lib:/usr/local/apr-util/lib:/usr/local/pcre/lib' \
    >> /etc/profile.d/lib_common_env.sh
